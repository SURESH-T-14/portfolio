...
export class Stage {
  ...
  async loadTextures({ grunge, grain, floor }) {
    const [a, b, c] = await Promise.all([loadImage(grunge), loadImage(grain), loadImage(floor)]);
    upload(this.gl, this.tex.grunge, a);
    upload(this.gl, this.tex.grain, b);
    upload(this.gl, this.tex.floor, c);
  }
  ...
  render(state, time, clips) {
    ...
    // ---- 1 plate --------------------------------------------------------
    {
      ...
      gl.useProgram(p.p);
      gl.uniform1i(p.u.uFloor, bind(gl, this.tex.floor, 0));
  ...
  _heroFigure(clip, geo, st, skew) {
    ...
    gl.uniform1f(p.u.uPacked, clip.format === 'video' ? 0 : 1);
    gl.uniform1i(p.u.uGrunge, bind(gl, this.tex.grunge, 1));
